...
import { Plus, Search, Filter, FileText, CheckCircle, AlertTriangle, AlertCircle, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function ProcesosPage() {
  ...
  const loadProcesos = async () => {
    try {
      setLoading(true)
      setError(null)
      const data = await procesosAPI.getAll({ limit: 1000 })
      setAllProcesos(data)
      setCurrentPage(1)
    } catch (err) {
  ...
  const handleMarkReviewed = async (procesoId: number, expediente: string) => {
    try {
      ...
      setAllProcesos(prev => prev.map(p => 
        p.id === procesoId 
  ...
  const filteredProcesses = allProcesos.filter((proceso) => {
    const matchesSearch =
      searchQuery === "" ||
      ...
      {loading ? (
        ...
        <Card className="p-6 sm:p-8">
          <EmptyState
            ...
            description={allProcesos.length === 0 ? "No hay procesos registrados aún" : "No hay procesos que coincidan con los filtros seleccionados"}
            action={{
