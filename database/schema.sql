...
CREATE TABLE notificaciones (
  id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
  tipo_alerta ENUM('audiencia_3d','impulso_25d','impulso_30d','plazo_7d','plazo_3d','plazo_hoy') NOT NULL,
  ...
  destinatario VARCHAR(190) NOT NULL, -- email o número
  asunto VARCHAR(190),
  cuerpo TEXT,
  estado_envio ENUM('pendiente','enviado','fallido') NOT NULL DEFAULT 'pendiente',
  intento TINYINT UNSIGNED NOT NULL DEFAULT 0,
  error_msg TEXT,
  scheduled_at DATETIME NULL,
  sent_at DATETIME NULL,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  KEY idx_notif_estado (estado_envio),
  KEY idx_notif_tipo (tipo_alerta),
  CONSTRAINT fk_notif_aud FOREIGN KEY (audiencia_id) REFERENCES audiencias(id) ON DELETE SET NULL,
  CONSTRAINT fk_notif_proc FOREIGN KEY (proceso_id) REFERENCES procesos(id) ON DELETE SET NULL,
  CONSTRAINT fk_notif_res FOREIGN KEY (resolucion_id) REFERENCES resoluciones(id) ON DELETE SET NULL
) ENGINE=InnoDB;
